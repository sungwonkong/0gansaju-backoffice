...
//컨트랙트 주소
const ContractAddress = alchemyConfig.metakongzContract;
...
const Dashboard = () => {
  ...
  async function getOpenseaTradeData() {
    ...
    for (const key in openseaTradeData.nftSales) {
      ...
      timestamp = await fetch(
        etherScanConfig.api.getblockreward(blockNumber) +
          etherScanConfig.apiToken,
  ...
  async function getOwnersForCollection() {
    ...
    const totalRes = await alchemy.nft.getContractMetadata(ContractAddress);

    setTotalCnt(Number(totalRes.totalSupply));
    //데이터 넣기
    setToken([]);
        ...
        <div className="stats shadow justify-center items-center">
          ...
          <div className="stat">
            ...
            <div className="stat-value">
              {openseaTradeCnt} {openseaTradeCnt == 1000 ? "+" : ""}
            </div>
          </div>
        </div>
      </>
  ...
  function RenderToken({ token }: any) {
    //console.log(token)
    const urlNft = token.imageURL.replace("ipfs://", "");
  ...
  useEffect(() => {
    ...
    setToken([]);
    setCollectionImg(
      "https://i.seadn.io/gcs/files/39ed06f47fe9566feabd5ede39253a7c.png?auto=format&w=384",
